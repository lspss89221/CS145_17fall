...
def applyZScore(dataframe):
    ...
    # normalized_dataframe.values = dataframe.values
    # (dataframe.values - dataframe.values.mean(0))/dataframe.values.std()
    # print dataframe.values.mean(0)
    # print dataframe.values.std()
...
def getBetaBatchGradient(train_x, train_y, alpha):
    ...
    new_beta = np.random.rand(train_x.shape[1])
    ...
    while True: 
        ...
        constant = np.dot(train_x,beta)-train_y
        step = np.dot(train_x.transpose(),constant)
        new_beta = beta - alpha*step
        
        new_predicted_y = np.dot(train_x,new_beta)
        new_mse = (np.square(new_predicted_y-train_y)).mean()
        
        if(new_mse*10000000 == mse*10000000):
            return new_beta
...
def getBetaStochasticGradient(train_x, train_y, alpha):
    ...
    while True: 
        for i in range(0,train_x.shape[0]):
            constant = train_y[i] - np.dot(train_x[i],beta)  
            beta = beta + alpha * np.dot(train_x[i].transpose(),constant)
            # print new_beta

        new_predicted_y = np.dot(train_x,beta)
        new_mse = (np.square(new_predicted_y-train_y)).mean()
        
        if(math.fabs(new_mse-mse) <= 0.000001):
            return beta
        # print new_mse
        mse = new_mse
...
def compute_mse(predicted_y, test_y):
    mse = 100.0
    ########## Please Fill Missing Lines Here ##########
    mse = (np.square(predicted_y-test_y)).mean()
