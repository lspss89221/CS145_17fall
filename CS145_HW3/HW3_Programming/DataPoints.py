...
class DataPoints:
    ...
    def getMean(clusters, mean):
        ...
        for cluster in clusters:
            ...
            for point in cluster:
                mean[idx][0] = mean[idx][0] + point.x
                mean[idx][1] = mean[idx][1] + point.y
            mean[idx][0] = mean[idx][0]/(len(cluster)) 
            mean[idx][1] = mean[idx][1]/(len(cluster))
            idx = idx + 1
        pass
    # -------------------------------------------------------------------
    @staticmethod
    def getStdDeviation(clusters, mean, stddev):
        ...
        for cluster in clusters:
            ...
            stddev[idx][1] = 0
            for point in cluster:
                stddev[idx][0] = stddev[idx][0] + (point.x - mean[idx][0])*(point.x - mean[idx][0])/(len(cluster))
                stddev[idx][1] = stddev[idx][1] + (point.y - mean[idx][1])*(point.y - mean[idx][1])/(len(cluster))
            stddev[idx][0] = math.sqrt(stddev[idx][0])
            stddev[idx][1] = math.sqrt(stddev[idx][1])
            idx = idx + 1
        pass
    # -------------------------------------------------------------------
    @staticmethod
    def getCovariance(clusters, mean, stddev, cov):
        ...
        idx = 0
        for cluster in clusters:
            cov[idx][0][0] = stddev[idx][0]**2
            cov[idx][0][1] = 0
            cov[idx][1][0] = 0
            cov[idx][1][1] = stddev[idx][1]**2
            for point in cluster:
                cov[idx][0][1] = cov[idx][0][1] + (point.x - mean[idx][0])*(point.y - mean[idx][1])/(len(cluster))
                cov[idx][1][0] = cov[idx][1][0] + (point.x - mean[idx][0])*(point.y - mean[idx][1])/(len(cluster))
            idx = idx + 1
        pass
    # -------------------------------------------------------------------
    @staticmethod
