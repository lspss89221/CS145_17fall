...
def find_K_neighbours(train_x, train_y, test_x_j, K):
    ...
    ########## Please Fill Missing Lines Here ##########
    distances = np.full(len(train_x),-1, dtype=int)
    for i in range(len(train_x)):
        distances[i] = euclidean_distance(train_x[i],test_x_j)
    index = np.argsort(distances)

    for i in range(K):
        neighbours_y[i] = train_y[index[i]]
    # print "Neighbours: "
    # print neighbours_y
    return neighbours_y
...
if __name__ == '__main__':
    ...
    plt.plot(range(1,120),average_accuracies, 'b')
    plt.axis([0, 120, 0, 1])
    plt.show()
